...
def data_generator(n = 10, num_ships = 20, num_flights = 20, num_cargos = 100):
    random.seed(1)
    sea_data_generator(n, num_ships)
    air_data_generator(n, num_flights)
    cargo_data_generator(n, num_cargos)


def sea_data_generator(n, num_ships):
    def sea_arc_time_cost():
        arc_file = open("sea%s_arccost.txt" % str(n),'w')
        time_file = open("sea%s_timecost.txt" % str(n),'w')
        ...
        time_file.close()
    def sea_stop_cost() :
        stop_file = open("sea%s_stopcost.txt" % str(n),'w')
    ...
    def sea_ships_param(num_ships) :
        param_file = open("sea%s_ships_param.txt" % str(n),'w')
        ...
        for i in range(num_ships):
            ...
            param_file.write(node + '\t' + str(starting_time) + '\t' + str(freq) + '\t' + str(cycle_time) + '\n')
        param_file.close()
...
def air_data_generator(n, num_flights):
    def air_arc_time_cost():
        arc_file = open("air%s_arccost.txt" % str(n),'w')
        time_file = open("air%s_timecost.txt" % str(n),'w')
        arc_file.write(str(n) + '\n')
        time_file.write(str(n) + '\n')
        ...
        time_file.close()
    def air_stop_cost() :
        stop_file = open("air%s_stopcost.txt" % str(n),'w')
        stop_cost = [random.randint(15,30) * 10 for _ in range(n)]
        for i in range(n) :
            stop_file.write(str(stop_cost[i]))
            if i != n - 1 :
    ...
    def air_ships_param(num_flights) :
        param_file = open("air%s_flights_param.txt" % str(n),'w')
        param_file.write(str(num_flights) + '\n')

        for i in range(num_flights):
            node = chr(65 + random.randint(0,n-1))

            cycle_time = random.randint(4,8)
            gap = cycle_time + numpy.random.poisson(0.5) + 1

            freq = floor(15 / gap)
            param_file.write(node + '\t' + str(gap) + '\t' + str(freq) + '\t' + str(cycle_time) + '\n')
        param_file.close()
...
def cargo_data_generator(n,num_cargos):
    cargo_file = open("cargo%s.txt"% str(n), 'w')
